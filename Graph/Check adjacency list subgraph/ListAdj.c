...
void F_aggiungi_arco_grafo_lista(Grafo *G, int valoreNodoPartenza, int valoreNodoArrivo, int peso){
    ...
    if(!F_struttura_vuota(nodoPartenza)){
        ...
        if(!F_struttura_vuota(nodoArrivo)){
            F_inserisci_arco_grafo_lista(&nodoPartenza->arcoPtr,nodoArrivo->nodo,peso);
...
ListaAdj F_cerca_arco_grafo_lista(ListaAdj *L, int valoreNodoDaTrovare){
    if(F_struttura_vuota(*L)) return NULL;
    if((*L)->nodo==valoreNodoDaTrovare) return (*L);
    return F_cerca_nodo_grafo_lista((&(*L)->arcoPtr),valoreNodoDaTrovare);
}
...
void F_inserisci_arco_grafo_lista(ListaAdj *L, int valoreDaInserire, int peso){
    if(F_struttura_vuota(*L)) {
        Peso P=NULL;
        F_alloca_nodo_grafo_lista(L,valoreDaInserire);
        F_alloca_struttura_peso(&P);
        P->peso=peso;
        (*L)->pesoPtr=P;
    }
    else{
        if((*L)->nodo==valoreDaInserire)  { printf("Arco con (%d) gia' presente.\n",valoreDaInserire); return;}
        else F_inserisci_arco_grafo_lista((&(*L)->arcoPtr),valoreDaInserire,peso);
    }
}

void F_alloca_struttura_peso(Peso *P){
    if(F_struttura_vuota(*P)){
        (*P)=(struct struttura_grafo_pesato*)malloc(sizeof(struct struttura_grafo_pesato));
        (*P)->peso=0;
    }
}
...
void F_stampa_grafo_arco(ListaAdj *L){
    if(!F_struttura_vuota(*L)){
        printf("-[%d]->(%d)",(*L)->pesoPtr->peso,(*L)->nodo);
        F_stampa_grafo_arco(&(*L)->arcoPtr);
    }
}
